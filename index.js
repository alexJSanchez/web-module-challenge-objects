...
function addReview(myArray,name,rating,feedback){
  myArray.push({name,rating,feedback});
  return myArray;
}
...
return `${myArray[lastIndex].name} gave the restaurant a ${myArray[lastIndex].rating} star review, and their feedback was: ${myArray[lastIndex].feedback}`;
} 
...
*/
 ...
 function getReviewByRating(myArray,ratingNum) {
    let secondArray = [];
    for (let i = 0;i < myArray.length;i++){
      if (myArray[i].rating === ratingNum){
...
function getLongReviews(myReviewsArray) { 
  let newArray = [];
    for (let i = 0;i < myReviewsArray.length;i++){
     let reviewObject = myReviewsArray[i];
      let wordCount = reviewObject.feedback.split(" "); 
